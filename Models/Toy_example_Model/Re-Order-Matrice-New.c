...
{
    if (argc != 2) {
        fprintf(stderr, "Usage: ./reordonner <modelName>   (ex: ./reordonner model50)\n");
        return 1;
    }
    ...
    snprintf(sz,     sizeof(sz),     "%s.sz",           argv[1]);
    snprintf(cd,     sizeof(cd),     "%s.cd",           argv[1]);
    snprintf(rii,    sizeof(rii),    "%s.Rii",          argv[1]);
    snprintf(pi,     sizeof(pi),     "%s.pi",           argv[1]);

    snprintf(szout,  sizeof(szout),  "%s-reordre.sz",   argv[1]);
    snprintf(cdout,  sizeof(cdout),  "%s-reordre.cd",   argv[1]);
    snprintf(part,   sizeof(part),   "%s-reordre.part", argv[1]);
    snprintf(riiout, sizeof(riiout), "%s-reordre.Rii",  argv[1]);
    snprintf(piout,  sizeof(piout),  "%s-reordre.pi",   argv[1]);
