...
/* -- lire une chaine dans un buffer  --*/
 void lire_chaine(char **deb,char nombre[150]) 
 {
 char *fin;
 fin=*deb;
 if (*fin==' ')
...
void  Construire_Matrice_Transition(char *fich) 
{
	...
	if((ligne_matrice=(char *)calloc((N+1)*100,sizeof(char)))==NULL) {printf("pas de mem \n");exit(1);}
...
/* -- Ecriture de la distribution stationnaire dans un fichier */
	if((fic_resultat= (char *) calloc ( (300) , sizeof(char)))==NULL) {printf("pas de mem \n");exit(1);}
