...
int Buffer, idAction, NbEvtsPossibles;
double Action[NbRelease];
...
void ReadAction(int idAction)
{
	// "Actions.txt" stocké à la racine de "NREL_Model"
    FILE *f = fopen("Actions.txt", "r");
    if (!f) { perror("Actions.txt"); exit(EXIT_FAILURE); }

    int N, K;

    // 1) Lire N et K
    if (fscanf(f, "%d", &N) != 1) { fprintf(stderr, "Erreur lecture N\n"); exit(EXIT_FAILURE); }
    if (fscanf(f, "%d", &K) != 1) { fprintf(stderr, "Erreur lecture K\n"); exit(EXIT_FAILURE); }

    if (K != NbRelease) {
        fprintf(stderr, "Incohérence: K fichier=%d, NbRelease=%d\n", K, NbRelease);
        exit(EXIT_FAILURE);
    }

    if (idAction < 0 || idAction >= N) {
        fprintf(stderr, "idAction=%d hors bornes [0..%d]\n", idAction, N-1);
        exit(EXIT_FAILURE);
    }
    ...
    // 2) Sauter les lignes 0..idAction-1
    for (int i = 0; i < idAction; i++) {
        if (fscanf(f, "%d", &id) != 1) { fprintf(stderr, "Erreur skip id\n"); exit(EXIT_FAILURE); }
        for (int j = 0; j < K; j++) {
            if (fscanf(f, "%lf", &tmp) != 1) { fprintf(stderr, "Erreur skip proba\n"); exit(EXIT_FAILURE); }
        }
    }

    // 3) Lire la ligne cible
    if (fscanf(f, "%d", &id) != 1) { fprintf(stderr, "Erreur lecture id\n"); exit(EXIT_FAILURE); }

    // sécurité (optionnelle)
    if (id != idAction) {
        fprintf(stderr, "Fichier incohérent: attendu id=%d, lu id=%d\n", idAction, id);
        exit(EXIT_FAILURE);
    }

    for (int j = 0; j < K; j++) {
        if (fscanf(f, "%lf", &Action[j]) != 1) {
            fprintf(stderr, "Erreur lecture Action[%d]\n", j);
            exit(EXIT_FAILURE);
        }
    }

	for (int i = 0; i < NbRelease; i++)
		printf("Sous-Action %d : %lf \n",i,Action[i]);
    fclose(f);
}
...
void  usage()
{
	printf("usage : GenerMatrix -f filename bufferSize idAction cityName \n");
...
char *argv[];
{
	...
	switch (*++*argv) {
		...
		{++argv;
			...
			printf("\n ================ %s: Buffer = %d, idAction = %d ==================\n",path,Buffer,idAction);
			ReadAction(idAction);
			NbEvtsPossibles = ReadDistribs(Buffer, city);
