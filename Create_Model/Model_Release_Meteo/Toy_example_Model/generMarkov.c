...
void lectureAction(int idAction)
{
...
void  usage()
{
	printf("usage : GenerMatrix -f filename deadline BufferSize idAction \n");
...
char *argv[];
{
	...
	switch (*++*argv) {
		...
		{++argv;
			...
			Buffer = atoi (*(++argv));
			idAction = atoi (*(++argv));
			printf("\n ================ Deadline = %d, Buffer = %d, idAction= %d ==================\n",Deadline,Buffer,idAction);
			lectureAction(idAction);
			pf1=fopen(s1,"w");
			pf2=fopen(s2,"w"); 
			break;
		}
