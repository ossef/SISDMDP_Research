...
def policy_Improvement_Csr(P, Ria, policy, H, N, A, d_factor ): 
    ...
    Q = np.zeros((N, A))
    for a in range(A):
        Q[:, a] = np.round(Ria[:, a] + d_factor*P[a].dot(H),7)  # à mettre à 10
...
def average_Measures_Teste(
    P, states, Parts, policy, N,
    packet_size, Buffer, n_packets,
    h_deb, h_fin, sell_probs,
    pService, weather_hours_packets,
    X_min=None
):
    ''' 
        Average measures for the SIMPA journal model: PV model (M, D, H, X)

        Requires:
          - optimal policy
          - transition matrices P (list/array-like: P[a] is CSR)
          - other model parameters

        Computes:
          - mean Energy released (sold_mean)
          - mean Energy losses   (loss_mean)
          - probability of Delay (noService)
          - (NEW) mean reserve deficit E[max(0, X_min - X)] in energy units
    '''

    # Build TPM induced by policy
    matrix_policy_rows = [P[policy[s]][s] for s in range(N)]
    matrix_policy = vstack(matrix_policy_rows)
    pi = steady_State_Chiu_Rob(matrix_policy, Parts)
    ...
    loss_mean = 0.0
    noService = 0.0
    reserve_deficit_mean = 0.0

    for i, (m, d, h, x) in enumerate(states):
        a = policy[i]

        # ---- NEW: Reserve deficit (stationary, current-state) ----
        reserve_deficit_mean += pi[i] * max(0, X_min - x) * packet_size

        # ---- Energy Packet Release (energy sold) ----
        if (h == h_fin[m]) or (x == Buffer):
            p_full, p_half, p_quarter = sell_probs[a]
            sold = (p_full * x) + (p_half * (x // 2)) + (p_quarter * (x // 4))
            sold_mean += pi[i] * sold * packet_size

        # ---- Energy Packet Loss (overflow) ----
        if x > Buffer - (n_packets - 1):
            hh = h - h_deb[m]
            ps = pService[hh]
            for e in range(n_packets):
                pe = weather_hours_packets[m][hh][e]
                # b=1 (service happens): prob ps
                loss_mean += pi[i] * pe * ps * max(0, x + e - 1 - Buffer) * packet_size
                # b=0 (no service): prob 1-ps
                loss_mean += pi[i] * pe * (1 - ps) * max(0, x + e - 0 - Buffer) * packet_size

        # ---- Data Packet Delay (NoService) ----
        if x == 0:
            hh = h - h_deb[m]
            noService += pi[i] * pService[hh]

    return sold_mean, loss_mean, noService, reserve_deficit_mean

def average_Measures(P, states, Parts, policy, N,
                     packet_size, Buffer, n_packets,
                     h_deb, h_fin, sell_probs,
                     pService, weather_hours_packets,
                     print_action_counts=True):
    ''' 
        - Average Measures for the SIMPA journal model: PV model (M, D, X, H) 
        - Need optimal policy and other model parameters 
        - Calculate mean Energy released E[Release]
        - Calculate mean Energy losses   E[loss]
        - Calculate probability of Delay P(NoService)
        - PLUS: print/count actions used per regime at decision (release) states h == h_fin[m]
    '''
    ...
    # --- Action counts per regime (only decision/release states) ---
    action_counts_by_M = defaultdict(Counter)

    # Ensure arrays for fast indexing if needed
    # (States may be list of tuples; policy may be list/np.array)
    ...
    for i, (m, d, h, x) in enumerate(states):
        ...
        # ---- Count actions ONLY at end-of-day decision states ----
        if h == h_fin[m]:
            action_counts_by_M[m][a] += 1

            # ---- Energy Packet Release (energy sold) ----
            p_full, p_two_thirds, p_one_third, p_not = sell_probs[a]
            sold = (p_full * x) + (p_two_thirds * ((2 * x) // 3)) + (p_one_third * (x // 3)) + (p_not * 0)
            sold_mean += pi[i] * sold * packet_size

        # ---- Energy Packet Loss (overflow) ----
        # possible only if x + (n_packets-1) > Buffer
        if x > Buffer - (n_packets - 1):
            hh = h - h_deb[m]
            ps = pService[hh]
            for e in range(n_packets):
    ...
    # Pretty print
    if print_action_counts:
        Ms = sorted(action_counts_by_M.keys())
        for M in Ms:
            counts = dict(sorted(action_counts_by_M[M].items(), key=lambda kv: kv[0]))
            # Format: Régime 0: {0: 13 fois, 13: 200 fois}
            inside = ", ".join([f"{a} : {c} fois" for a, c in counts.items()])
            print(f"Régime {M}: {{ {inside} }}")

    return sold_mean, loss_mean, noService
